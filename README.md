...
**Install Python, dependencies and executables:**

There are two ways to do this. Both end up with the same pinned versions of
pyEMU, FloPy and PEST++; pick whichever you are more comfortable with.

*Option A - pixi (recommended)*

[pixi](https://pixi.sh) manages the python environment *and* the
MODFLOW/PEST++ executables, and pins the entire transitive dependency set in
`pixi.lock`, so everyone gets an identical environment. You do not need conda,
and you do not need to install python yourself.

 - Install pixi by following the directions here: [pixi installation](https://pixi.sh/latest/#installation).
 - Open a terminal (on __Windows__, use PowerShell or the Command Prompt) and
   navigate to the course repo folder with `cd *your folder path*`.
 - `pixi install` - creates a self-contained environment inside the repo (in a
   `.pixi` folder). It may take a while.
 - `pixi run get-exes` - downloads MODFLOW 6, MODPATH 7, ZONEBUDGET 6 and the
   PEST++ suite into a `bin` folder in the repo.

Check the executables arrived with `pixi run check-exes`. Start jupyter with
`pixi run jupyter`.

*Option B - conda / mamba*

 - If you have already installed Python using Anaconda, you can skip this step.
   If not, install [miniforge](https://github.com/conda-forge/miniforge?tab=readme-ov-file#download).
 - If you are using __Windows__: go to the start menu and open "Anaconda
   prompt". On __Linux__ or __MacOS__, just use the standard terminal. Navigate
   to the course repo folder with `cd *your folder path*`.
...
Nothing is installed system-wide by either option, and the binaries are not
stored in the repo itself.

A JupyterLab instance should start within the course repo folder. Using the browser, you can now navigate to the "tutorials" folder, open a notebook, and you are in business!
