...
def henry_domain(figsize=(15,10)):
	...
	ax.scatter(hx, hz, marker='o', edgecolor='k', facecolor="none", s=20, label="head obs")
	ax.scatter(cx, cz, marker='x', color='k', s=10, label="concentration obs")
	ax2.scatter(cx[9], cz[9], marker='x', color='k', s=10)
	...
	for xx, zz, nn in zip(hx, hz, hnames):
		...
		if "10" in nn:
			ax2.text(xx,zz+0.125,nn[-2:],ha="center",va="bottom",fontsize="small",
    				bbox={"fc":"1.0","boxstyle":"round,pad=0.01","ec":"none"})
	...
	ax.plot([0,0],[0,0],lw=1,color='0.5',label="10% saltwater")
	...
	ax2.annotate(
	    '', xy=(0.095, -1.9), xycoords='data',
	    xytext=(9.0, -1.9), textcoords='data',
...
fig = henry_domain(figsize=(6,3.5))
